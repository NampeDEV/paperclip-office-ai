...
import { saveOfficeSceneSchema } from "@paperclipai/shared";
...
export function officeRoutes(db: Db, storage: StorageService) {
  ...
  router.get("/companies/:companyId/office-scene", async (req, res) => {
    ...
    res.json(await scenes.get(companyId));
  });
    ...
    try {
      ...
      if (error instanceof multer.MulterError) {
        if (error.code === "LIMIT_FILE_SIZE") {
          res.status(422).json({ error: "Background is larger than the 5 MiB limit" });
          return;
        }
